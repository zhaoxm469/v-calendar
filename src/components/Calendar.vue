                ...
                h(
                    ...
                    [
                        ...
                        h(
                            ...
                            {
                                class: [
                                    `vc-arrows-container title-${this.titlePosition_}`,
                                ],
                            },
    ...
    methods: {
        ...
        canMove(arg, opts = {}) {
            ...
            if (!page) {
                return Promise.reject(
                    new Error(`Invalid argument provided: ${arg}`),
                );
            }
        ...
        move(arg, opts = {}) {
            ...
            if (!opts.force && !canMove) {
                return Promise.reject(
                    new Error(
                        `Move target is disabled: ${JSON.stringify(opts)}`,
                    ),
                );
            }
            ...
            if (
                opts.fromPage &&
                !pageIsEqualToPage(opts.fromPage, this.firstPage)
            ) {
        ...
        refreshDisabledDays(pages) {
            this.getPageDays(pages).forEach(d => {
                d.isDisabled =
                    ...
                    this.disabledAttribute.intersectsDay(d);
            });
        },
        ...
        buildPage({ month, year }, ignoreCache) {
            ...
            if (!page || ignoreCache) {
                ...
                const prevMonthComps = this.$locale.getPrevMonthComps(
                    month,
                    year,
                );
                const nextMonthComps = this.$locale.getNextMonthComps(
                    month,
                    year,
                );
                page = {
                    ...
                    refresh: true,
                    minVisibleWeek: this.minVisibleWeek,
                };
                // Assign day info
